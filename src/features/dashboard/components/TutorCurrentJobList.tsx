...
const TutorCurrentJobList = () => {
  const { tutorCurrentJobStore } = useStoreContext(DashboardContext);
  const { jobConnections, pageInfo } = useSnapshot(tutorCurrentJobStore);
  const user = useUser();
  useEffect(() => {
    async function fetchData(tutorId: string) {
      await tutorCurrentJobStore.getTutorCurrentConnections(tutorId);
    }
  ...
  return (
    <>
      ...
      {pageInfo?.hasNextPage && (
        <Button
          onClick={() =>
            tutorCurrentJobStore.loadMoreTutorCurrentConnections(
              user?.tutorProfile.id ?? ""
            )
          }
        >
...
export default TutorCurrentJobList;
